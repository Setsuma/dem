...
namespace OnlineCinemaDB
{
    public partial class NewComment : Form
    {
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                commentsTableAdapter.Insert(text.Text.Trim(), fId, aId);
                MessageBox.Show("Новый комментарий добавлен");
                DialogResult = DialogResult.OK;
                this.Close();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Произошла ошибка при добавлении комментария");
            }
        }
